...
class SessionManager(context: Context) {
    ...
    // FIXED: Now accepts a forced break duration to override the original setup
    fun transitionToBreak(forcedBreakMinutes: Int? = null) {
        isBreakMode = true
        elapsedMinutes = 0f
        if (forcedBreakMinutes != null) {
            breakMinutes = forcedBreakMinutes
        }
    }
}