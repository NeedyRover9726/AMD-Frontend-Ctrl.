...
fun CtrlApp(
    ...
    var quizPurpose by remember { mutableStateOf("VERIFY") }
    ...
    // Helper to force a 4-second grace period so UsageStatsManager lag doesn't instantly re-lock you
    val triggerGracePeriod = {
        val graceIntent = Intent(context, AppBlockerService::class.java).apply {
            action = AppBlockerService.ACTION_FORCE_GRACE_PERIOD
        }
        context.startService(graceIntent)
    }

    // Helper to instantly tell the service that Break Mode has started
    val triggerSyncBreakMode = {
        val syncIntent = Intent(context, AppBlockerService::class.java).apply {
            action = AppBlockerService.ACTION_SYNC_BREAK_MODE
        }
    ...
    DisposableEffect(Unit) {
        registerIntentListener { dest ->
            if (dest == "intercept_input" || dest == "quiz") {
                quizPurpose = "VERIFY"
            }
    ...
    NavHost(navController = navController, startDestination = startDest) {
            ...
            HomeScreen(
                ...
                onDeleteMaterial = { material ->
                    coroutineScope.launch {
                        try {
                            val response = RetrofitClient.apiService.deleteMaterial(material.name)
                            if (response.status == "success") {
                                ...
                                Toast.makeText(context, "Deleted ${material.name}", Toast.LENGTH_SHORT).show()
                            }
                        } catch (e: Exception) {
                            Log.e("DeleteMaterial", "Failed to delete material from server", e)
                            Toast.makeText(context, "Failed to delete ${material.name}. Check connection.", Toast.LENGTH_SHORT).show()
                        }
                    }
                },
        ...
        composable("session_dashboard") {
            SessionDashboardScreen(
                ...
                onUpdate = {
                    quizPurpose = "UPDATE"
                    if (elapsedMinutes < 5f) {
                        navController.navigate("create_step_1")
                    } else {
                        navController.navigate("intercept_input")
                    }
                },
                onCancel = {
                    if (elapsedMinutes < 5f) {
                        ...
                        quizPurpose = "CANCEL"
                        navController.navigate("end_session_confirm")
                    }
                }
            )
        }
        ...
        composable("quiz") {
            ...
            val unpinAndDismissLock = {
                try {
                    ...
                    Log.e("AntiCheat", "Failed to gracefully unpin", e)
                }
            }
                ...
                onPass = {
                    ...
                    when (quizPurpose) {
                        ...
                        "UPDATE" -> {
                            readingProgress = 0.0f
                            isReadingStarted = false
                            navController.navigate("create_step_1") { popUpTo("home") }
                        }
                        else -> { // "VERIFY"
                            // FIXED: Instantly transition to break mode before we even navigate to the Unlock screen.
                            // This ensures apps are unblocked instantly even if the user never presses a button.
                            isBreakMode = true
                            sessionManager.transitionToBreak()
                            elapsedMinutes = 0f
                            triggerSyncBreakMode()
        ...
        composable("unlock") {
            UnlockScreen(
                breakTimeMins = breakMinutes, // FIXED: Passes dynamic actual break time
                elapsedMinutes = elapsedMinutes, // FIXED: Passes live elapsed time to show it counting down
                onOpenApp = {
                    triggerGracePeriod()
